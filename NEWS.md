# Changelog

## Karabiner-Elements-user-command-server v1.5.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.5.0/Karabiner-Elements-user-command-server-1.5.0.dmg)
- 📅 Release date
    - Jul 10, 2026
- ✨ New Features
    - Added the `center_focused_window` command.

## Karabiner-Elements-user-command-server v1.4.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.4.0/Karabiner-Elements-user-command-server-1.4.0.dmg)
- 📅 Release date
    - Jun 14, 2026
- ⚡️ Improvements
    - Updated dependent package: Karabiner-Elements-user-command-receiver v1.2.0.

## Karabiner-Elements-user-command-server v1.3.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.3.0/Karabiner-Elements-user-command-server-1.3.0.dmg)
- 📅 Release date
    - May 13, 2026
- ⚡️ Improvements
    - Support "left","right" in `x` and "top","center","bottom" in `y`.

## Karabiner-Elements-user-command-server v1.2.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.2.0/Karabiner-Elements-user-command-server-1.2.0.dmg)
- 📅 Release date
    - Feb 18, 2026
- ⚡️ Improvements
    - Updated dependent package: Karabiner-Elements-user-command-receiver v1.1.0.

## Karabiner-Elements-user-command-server v1.1.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.1.0/Karabiner-Elements-user-command-server-1.1.0.dmg)
- 📅 Release date
    - Feb 17, 2026
- ✨ New Features
    - The app now automatically adds itself to Open at Login.

## Karabiner-Elements-user-command-server v1.0.0

- [📦 Download](https://github.com/tekezo/Karabiner-Elements-user-command-server/releases/download/v1.0.0/Karabiner-Elements-user-command-server-1.0.0.dmg)
- 📅 Release date
    - Feb 16, 2026
- Initial release.
